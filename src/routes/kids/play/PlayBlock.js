...
import 'Css/kids/play/PlayBlock.css';
...
class PlayBlock extends PageView {
  constructor(props) {
    super(props);
  ...
  handleRequsetBlockInfo = async (menuId) => {
    ...
    console.log('%c[blocks] ===>', 'color:#0000ff ', blocks);
    ...
    switch (event.keyCode) {
      ...
      case keyCode.Keymap.ENTER:
        ...
        if (blockInfo.blkTypCd === kidsConfigs.BLOCK.CONTENTS_BLOCK_CD) {
          ...
          param.state = {
            menu_id: contentInfo.menuId,
            sris_id: contentInfo.shcutSrisId,
            epsd_id: contentInfo.shcutSrisId
          }
          this.setHistory({
            menuId: historyInfo.menuId,
            prevTitle: historyInfo.prevTitle,
  ...
  render() {
    ...
    console.log('============================');
    console.log('content:', content.prevTitle, content.currentTitle);
    console.log('============================');
