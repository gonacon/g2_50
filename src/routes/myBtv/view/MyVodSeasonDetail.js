...
import { G2NaviSlider, G2SlideMovie, G2SlideActorImage, G2SlideActorText, G2SlideWatchingVOD, SlideType } from 'Module/G2Slider';
...
import { MeTV } from './../../../supporters/network/MeTV';
...
class MyVodSeasonDetail extends PageView {
    constructor(props) {
        super(props);
        this.state = {
            menuId: 'play',
    ...
    componentDidMount() {
        ...
        const bottomButtons = new FM({
            ...
            startIdx: 0,
            lastIdx: 1,
            onFocusChild: this.onFocusBottomButton,
    ...
    update = async ({epsdId, srisId, prodId}) => {
        ...
        console.log('시즌 : result010:', result010);
        ...
        const seriesList = (Array.isArray(series) && series.length !== 0)? series.map((vod, idx) => {
            return {
                epsdId: vod.epsd_id,
                img: vod.poster_filename_v,
                title: vod.brcast_tseq_nm
            }
        }): null;

        this.buttonMenuIdList.push('play');
        if (seriesList) {
            this.buttonMenuIdList.push('series');
        }
        if (specialMovieList) {
        ...
        this.setState(newState, () => {
            this.setFocusEnable('seriesMovieList', false);
    ...
    onSelectPlay = async () => {
        ...
        let epsdId = this.vodInfo.epsdId, startTime = '';
        let paramData = {
            sris_id: this.vodInfo.srisId
        }
        const recentWatchVOD = await MeTV.request023(paramData);
        if (recentWatchVOD.result === '0000') {
            epsdId = recentWatchVOD.epsd_id;
            startTime = recentWatchVOD.watch_time;
        }
    ...
    onFocusBottomButton = (btnIdx) => {
        ...
        switch(this.state.menuId) {
            case 'series': this.setFocusEnable('seriesMovieList', false); break;
        ...
        this.setState({
            ...
            switch(id) {
                ...
                case 'series': this.setFocusEnable('seriesMovieList', true); break;
    ...
    onSelectSeriesMovie = (containerIdx, idx) => {
        const { seriesList } =  this.state;
        const series = seriesList[idx];
        if (series) {
            console.log('시리즈 ' + idx + '화 선택:', series);
            const playInfo = {
                fromCommerce: 'N',
                seeingPath: '21',   //소장용 VoD를 통한 시청
                epsd_id: series.epsdId,
                playOption: 'normal',
                playType: 'default',
                search_type: '1',
                cnr_id: '',
    ...
    getSeriesList = () => {
        const { seriesList: list } = this.state;
        if (!list) return null;
        return (
            <G2NaviSlider 
                id="seriesMovieList"
                type={SlideType.WATCHING_VOD}
                bShow={list.length !== 0}
                rotate={true}
                setFm={this.setFm}
                onSelectChild={this.onSelectSeriesMovie}
            >
            {
                list.map((vod, idx) => {
                    return (
    ...
    getButtonList = () => {
        const { specialMovieList, cornerMovieList, actorList, seriesList, menuId } = this.state;
        const buttonList = [];
        let buttonIdx = 0;
        buttonList.push( 
            <Button 
					...
					<div className="contentWrap">
						...
						<div className="synopShortContent">
							<div className="synopBot">
                                { menuId === 'series' && this.getSeriesList() }
...
export default MyVodSeasonDetail;
