 ...
 */
class BuyBillMonthly extends PopupPageView {
	constructor(props) {
		super(props);

		if (this.historyData == null) {
		let DATA_PPM = this.paramData;
		...
		this.state = {
			...
			DATA_DISCOUNT: {
				...
				TMS_INFO: {
					TMS_SAVED_CARD_YN: DATA_PPM.PROD_INFO.TMS_SAVED_CARD_YN,
					bLoaded : false,
				},
				TVPAY_INFO: {}
			},
		...
		const focusList = [
			...
			{ key: 'btnListBuyBillMonthly', fm: null }
		];
	...
	componentWillMount = async() => {
		...
		const { bRecoverMode, DATA_PPM } = this.state;
		if (bRecoverMode === false) {

			const requestEPS401 = EPS.request401({
				productId: DATA_PPM.PROD_INFO.PROD_DTL.PID
			});
			...
			Promise.all([requestEPS401, requestEPS300]).then((value) => {
				...
				if (value[0].result === '0000') {
					DATA_DISCOUNT.COUPON_INFO.COUPON_LIST = [];
					const price = Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE);
	...
	componentDidMount = () => {
		const btnListBuyBillMonthlyFm = new FM({
			id : 'btnListBuyBillMonthly',
			...
			startIdx : 0,
			lastIdx : 1,
			onFocusKeyDown: this.onKeyDownbtnListBuyBillMonthly,
			onFocusChild: this.onFocused
		});
		this.setFm('btnListBuyBillMonthly', btnListBuyBillMonthlyFm);
	...
	onFocusMoveUnavailable = (data) => {
		...
		if (data.id === 'billType' && data.direction === "RIGHT") {
		...
		} else if (data.id === 'btnListBuyBillMonthly' && data.direction === "LEFT") {
			// 결제 탭으로 이동
			const arrTargetpayment = this.state.DATA_PPM.PROD_INFO.TARGET_PAYMENT.split(",");
		...
		} else if (data.id === 'btnListBuyBillMonthly' && data.direction === "UP") {
			// Do nothing
			...
			this.state.focusView = 0;
			const targetpayment = this.state.DATA_PPM.PROD_INFO.TARGET_PAYMENT.split(",");
	...
	onKeyDownbtnListBuyBillMonthly = (evt, idx) => {
		...
		if (info.closed === 'Y' && info.resultCode === '0') {
			Core.inst().showPopup(<BillCertify />, { mode: 'mode_ppm' }, (info) => {
				if (info.result) {
					...
					discount.tvpayAmount = Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE) * 1.1;
					discount.usePhone = false;

					discount.totalPrice = discount.tvpayAmount;
					CTSInfo.requestPurchaseEPS111({ mode: DATA_PPM.mode, data: DATA_PPM, discountInfo: discount });
				}
			});
	...
	purchaseCallBack = (info) => {
		if (info.result) {
			let funcPurchase;
			const { typeSelected, discountInfo, DATA_PPM, tvPayInfo } = this.state;

			let discount = {};
			if (typeSelected === constants.PAYMENT_TYPE_CD.CREDIT) {
				// TV페이인 경우
				funcPurchase = CTSInfo.requestPurchaseEPS112;
				discount.useCoupon = false;
				discount.useTmembership = false;
				discount.useOcb = false;
				discount.useTvpoint = false;
				...
				if (!_.isEmpty(discountInfo.tmsInfo) && !_.isEmpty(discountInfo.couponInfo)) {
					// T멤버쉽 Coupon 모두 사용하는 경우 (T멤버쉽이 포함되면 EPS-112 호출)
					funcPurchase = CTSInfo.requestPurchaseEPS112;
					...
					discount.usePhone = false;
					discount.totalPrice = (Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE) + discountInfo.totalDiscount) * 1.1;
				} else if (!_.isEmpty(discountInfo.tmsInfo)) {
					// T멤버쉽 사용 하는 경우 (T멤버쉽이 포함되면 EPS-112 호출)
					funcPurchase = CTSInfo.requestPurchaseEPS112;
					...
					let totalDiscount = Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE);
					...
					discount.totalPrice = Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE) * 1.1;
					funcPurchase = CTSInfo.requestPurchaseChannel;
				}
			}

			if (DATA_PPM.mode === CTSInfo.MODE_PPM_HOME && _.isEmpty(discountInfo.tmsInfo)){
				// 홈 월정액인 경우, SCS-006 사용
				const data = {
					mode: DATA_PPM.mode,
					data: DATA_PPM,
					paymethod: typeSelected,
					discountInfo: discount
				}
				CTSInfo.requestPurchaseHomePPM(data);
			} else {
				funcPurchase({data: DATA_PPM, paymethod: typeSelected, discountInfo: discount});
			}
		}
	}

	billTypeKeyDown = (keyCode, targetPayment, tpIndex) => {
		let bShowOptionView, arrTargetpayment = this.state.DATA_PPM.PROD_INFO.TARGET_PAYMENT.split(",");
		if (keyCode === keyCodes.Keymap.ENTER) {
			...
			// 결제하기 버튼으로 이동
			this.setFocus('btnListBuyBillMonthly');
	...
	updateDiscountInfo = () => {
		const { DATA_PPM, focusView, focusIdx } = this.state;
			...
			switch (obj.type) {
				case "01":
					if (Number(DATA_DISCOUNT.COUPON_INFO.COUPON_LIST.length) < 1) {
                        ...
                        Core.inst().showPopup(<PopupConfirm />, param, (info) => {
                            if (info.result) {
								const { DATA_PPM, selectedCouponNo } = this.state;
								Core.inst().showPopup(<BillCoupon />, { pid: DATA_PPM.PROD_INFO.PROD_DTL.PID, selectedCouponNo: selectedCouponNo }, this.callbackBillCoupon);
							}
						});
					}
					break;
			...
			switch (obj.type) {
				case DISCOUNT_COUPON:
					if (Number(DATA_DISCOUNT.COUPON_INFO.COUPON_LIST.length) > 0) {
						if (selectedInfo[obj.type] === 1) {
							// 할인 적용
							if (Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE) + discountInfo.totalDiscount <= 0) {
				...
				case DISCOUNT_TMS:
					if (DATA_DISCOUNT.TMS_INFO.TMS_SAVED_CARD_YN === "N" || DATA_DISCOUNT.TMS_INFO.TMS_SAVED_CARD_YN === "") {
						...
						const data = await EPS.request501({ productId: this.state.DATA_PPM.PROD_INFO.PROD_DTL.PID });
						if (data.result === '0000' && data.discountRate !== undefined) {
							...
							if (selectedInfo[obj.type] === 1 && Number(DATA_PPM.PROD_INFO.PROD_DTL.PRICE) + discountInfo.totalDiscount <= 0) {
								selectedInfo[obj.type] = 0;
								Core.inst().showToast('할인금액이 구매금액보다 큽니다.\n선택되어 있는 다른 할인 수단을 해제해 주세요.');
							} else {
	...
	callbackBillCoupon = (info) => {
		...
		.then(data => {
			...
			if (data.result === '0000') {
				let resultOk = false;
				const price = Number(this.state.DATA_PPM.PROD_INFO.PROD_DTL.PRICE);
				for (let i = 0; i < DATA_DISCOUNT.COUPON_INFO.COUPON_LIST.length; i++) {
					if (data.coupon.couponNo === DATA_DISCOUNT.COUPON_INFO.COUPON_LIST[i].couponNo) {
						resultOk = true;
						DATA_DISCOUNT.COUPON_INFO.COUPON_LIST[i].discountAmount = data.coupon.discountAmount;
	...
	goToPurchaseTvPay() {
		...
		Core.inst().showPopup(<BillCertify />, { mode: 'mode_ppm' }, (info) => {
			if (info.result) {
				const { DATA_PPM, DATA_DISCOUNT } = this.state;
				const paramData = {
					PRICE: DATA_PPM.PROD_INFO.PROD_DTL.PRICE,
					PNAME: DATA_PPM.PROD_INFO.PROD_DTL.PNAME,
	...
	goToPurchasePhone() {
		...
		Core.inst().showPopup(<BillCertify />, { mode: 'mode_all' }, (info) => {
			if (info.result) {
				const { mode, DATA_PPM } = this.state;
				const data = {
					mode: mode,
					PID: DATA_PPM.PROD_INFO.PROD_DTL.PID,
					V_PRICE: Number( DATA_PPM.PROD_INFO.PROD_DTL.PRICE ) * 1.1 
				}
				Core.inst().showPopup(<BillPhoneCertify />, { data: data, DATA_API: DATA_PPM });
			}
		});
	}
	checkdiscountInfo() {
		const { DATA_PPM, typeSelected } = this.state;
		let discountInfo;
		if (typeSelected !== constants.PAYMENT_TYPE_CD.BILL) {
			discountInfo = {
				totalDiscount: 0,
	...
	render() {
		...
		conuntOptionList = DATA_PPM.PROD_INFO.TARGET_DISCOUNT.split(",");
		...
		let paymentType = [];
		let targetpayment = DATA_PPM.PROD_INFO.TARGET_PAYMENT.split(",");
		for (let i = 0; i < targetpayment.length; i++) {
		...
		return (
			<div className="buyWrap">
				<div className="buyTop">
					<span>월정액 구매</span>
					<div>{DATA_PPM.PROD_INFO.PROD_DTL.PNAME}</div>
					{DATA_PPM.enterance === 2 &&
				...
				<div className="buyContent buyBill">
					...
					<div className="right">
						<PurchaseDetailView mode={mode} data={DATA_PPM} totalDiscount={totalDiscountInfo.totalDiscount} selectedInfo={selectedInfo} />
						<div id="btnListBuyBillMonthly"><PurchaseBtnView /></div>
						{ !this.state.bShowOptionView &&
		...
		return (
			<div className="optionView">
				<ul>
					<li>
						<span>월정액</span>
						<span className="optionList">{data.PROD_INFO.PROD_DTL.PNAME}</span>
					</li>
					<li>
						<span>이용기간</span>
						<span className="optionList">{data.PROD_INFO.PROD_DTL.DUETIME_STR}</span>
					</li>
				</ul>
					...
					<div className="priceWrapper">
						<div className="priceInner">
							<div className="subPrice">
								<span>상품원가</span>
								<span className="priceValue">{numberWithCommas(Number(data.PROD_INFO.PROD_DTL.PRICE))}<em>원</em></span>
							</div>
							<div className="subPrice">
								<span>할인</span>
