...
class G2AEDContentSlider extends Component {
    constructor(props) {
        super(props);
    ...
    onFocusChanged = (idx, page) => {
        ...
        this.fm.listInfo.curIdx = focusedIdx;
        this.fm.listInfo.focusIdx = focusedIdx;
    ...
    componentDidMount() {
        const { id, idx, setFm, type, children, rotate, anchor, content } = this.props;
        const maxItem = SlideInfo[type].maxItem;
    ...
    render() {
        ...
        switch (type) {
            ...
            case SlideType.EVENT_COUPLE:
                wrapperClass = `slideWrap${focused ? ' activeSlide' : ''}${!isFirst ? ' firstActive' : ''}${isLastItem ? ' lastActive' : ''}${bLeftAffordance? ' leftActive': ''}${bRightAffordance? ' rightActive': ''}${isLastItem? ' lastActive':''}`;
...
export default G2AEDContentSlider;