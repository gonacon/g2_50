...
class G2BannerSlider extends Component {
    ...
    componentDidMount() {
        console.error('banner.didmount', this.props);
    ...
    componentWillReceiveProps(nextProps) {
        console.error('banner.receive', nextProps);
    ...
    createFakeElements = () => {
        ...
        const cloneList = React.Children.map(childList, (child, idx) => {
            ...
            return React.cloneElement(child, {
                ...
                idx, 
                key: idx, 
                ref: r=>saveRefs(this.fakeChildrenList, idx)
            });
        });
        ...
        const head = React.cloneElement(childList[lastIdx], {
            pos: childList.length, 
            idx: lastIdx, 
            key: 'head', 
            ref: r=>saveRefs(this.fakeChildrenList, childList.length)
        });
        const tail = React.cloneElement(childList[0], {
            pos:-1, 
            idx: 0, 
            ref: r=>saveRefs(this.fakeChildrenList, -1)
        });
                ...
                <div className={focusClass}>
                    <div className={containerFocusClass}>
                        <div className="slideCon">
                            <div className="slideWrapper" style={slideStyle} ref={r=>this.container=r}>
                                {fakeChildren}
                            </div>
                        </div>
